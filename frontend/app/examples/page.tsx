  ...
  return (
    <div className={styles.page}>
      ...
      <header className={styles.header}>
        ...
        <h1 className={styles.pageTitle}>Example Library</h1>
        <div />
      </header>

      <main className={styles.main}>
        <p className={styles.intro}>
          Curated AI-generated Python patterns. Each example explains{" "}
          <em>why</em> AI writes this code. Save any example to your review queue.
        </p>
        ...
        <div className={styles.tabs}>
          ...
          {categories.map((cat) => (
            <button
              ...
              style={activeCategory === cat ? { borderColor: getCategoryColor(cat), color: getCategoryColor(cat) } : {}}
            >
              {cat.replace("_", " ")}
            </button>
          ))}
        ...
        {error && (
          <div className={styles.error}>
            <span>⚠</span> {error}
            <button onClick={() => setPage(page)} className={styles.retryBtn}>Retry</button>
          </div>
        )}
