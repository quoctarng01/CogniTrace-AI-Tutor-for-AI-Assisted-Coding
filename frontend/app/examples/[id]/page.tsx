...
const TYPE_COLORS: Record<string, string> = {
  ...
  factory: '#22c55e',
  typevar: '#f59e0b',
  parameterized: '#06b6d4',
  generic_class: '#a855f7',
  callable_sig: '#3b82f6',
  nested_call: '#a855f7',
  coalesce: '#06b6d4',
  ternary: '#a855f7',
  ...
  config_layer: '#3b82f6',
  func_layer: '#a855f7',
  execution_layer: '#22c55e',
  init: '#6b7280',
  callable_layer: '#a855f7',
  abc: '#3b82f6',
  contract: '#f59e0b',
  boilerplate: '#6b7280',
  mutable_default: '#f97316',
  mixin: '#a855f7',
  mixin_usage: '#22c55e',
  self_reflection: '#06b6d4',
  enter: '#06b6d4',
  exit: '#f59e0b',
  timing: '#f59e0b',
  async_wait: '#06b6d4',
...
export default function ExampleDetailPage() {
      ...
      <div className={styles.page}>
        <header className={styles.topBar}>
          ...
          <nav className={styles.navLinks}>
            <Link href="/tracer" className={styles.navLink}>New Trace</Link>
            <Link href="/examples" className={`${styles.navLink} ${styles.navLinkActive}`}>Examples</Link>
          </nav>
        </header>
  ...
  const intervals = Array.isArray(example.review_interval)
    ...
    : String(example.review_interval)
        .split(',')
        .map(s => s.trim())
        .filter(Boolean);
      ...
      <main className={styles.main}>
                ...
                <div key={i} className={styles.annotation}>
                  ...
                  <span
                    ...
                    style={{
                      ...
                      backgroundColor: getTypeColor(ann.type) + '15',
                    }}
                  >
                    {ann.type}
                  </span>
        ...
        <section className={styles.saveSection}>
          {saveState === 'saved' ? (
            ...
            <div className={styles.errorBanner}>
              <span>⚠</span> {saveMessage}
              <button onClick={handleSave} className={styles.retryBtn}>
                Try Again
              </button>
            </div>
          ) : (
          ...
          <p className={styles.reviewHint}>Review interval: {intervals.join(' → ')} days</p>
        </section>
      </main>
    </div>
  );
