...
export default function DashboardPage() {
  ...
  useEffect(() => {
    async function load() {
      console.log("[DEBUG] Dashboard - starting load");
      
      // First try to restore session from localStorage
      await getAuthToken();
      
      const { data } = await getSupabase().auth.getSession();
      console.log("[DEBUG] Dashboard - session after getAuthToken:", data?.session ? "EXISTS" : "NULL");
      if (!data?.session) { 
      ...
      try {
        console.log("[DEBUG] Dashboard - calling fetchDashboard");
        const data = await fetchDashboard();
        console.log("[DEBUG] Dashboard - fetchDashboard returned");
        setTraces(data.traces);
        setDueCards(data.due_cards);
        setStreak(data.streak);
      } catch (err) {
        console.log("[DEBUG] Dashboard - fetchDashboard error:", err instanceof Error ? err.message : err);
        ...
        <section>
          ...
          {traces.length === 0 ? (
            <div className={styles.emptyState}>
              ...
              <p className={styles.emptyHint}>Paste a function and click &quot;Save&quot; to get started.</p>
            </div>
          ) : (
