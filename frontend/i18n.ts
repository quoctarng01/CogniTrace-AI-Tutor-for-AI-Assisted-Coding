...
export default getRequestConfig(async () => {
  ...
  const localeParts = firstLanguage.split('-');
  const preferredLocale = localeParts.length >= 2 && localeParts[0]
    ? (localeParts[0].toLowerCase() as Locale)
    : defaultLocale;
  ...
  const locale =
    localeCookie ?? (locales.includes(preferredLocale) ? preferredLocale : defaultLocale);
