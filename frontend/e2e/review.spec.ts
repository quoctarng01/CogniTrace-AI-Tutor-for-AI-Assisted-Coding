...
test.describe('Authenticated Dashboard', { tag: ['@auth'] }, () => {
  ...
  test('streak badge is visible in Review Queue section', async ({ page }) => {
    ...
    const hasStreakContent = await reviewSection
      .getByText(/streak|Start/i)
      .isVisible()
      .catch(() => false);
    const hasBadgeArea = await reviewSection
      .locator('[class*="streakBadge"]')
      .isVisible()
      .catch(() => false);
  ...
  test('shows review cards or empty state (not both missing)', async ({ page }) => {
    ...
    await page.waitForLoadState('networkidle');
    await expect(page.getByText(/Loading dashboard/i))
      .not.toBeVisible({ timeout: 5000 })
      .catch(() => {});

    const hasReviewCards = await page
      .getByRole('link', { name: /Review Now/i })
      .isVisible()
      .catch(() => false);
    const hasEmptyState = await page
      .getByText(/No reviews due/i)
      .isVisible()
      .catch(() => false);
...
test.describe('Review Flow', { tag: ['@auth', '@review'] }, () => {
  ...
  test('submitting a rating shows completion state with next review date', async ({ page }) => {
    ...
    await expect(page.getByText(/next review|due|day/i)).toBeVisible({ timeout: 5000 });
