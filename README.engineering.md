...
> **Engineering reference.** The research-framed README (positioning, contributions, AI architecture) is at [`README.md`](./README.md). This file is the engineering walk-through: pipeline, stack, configuration, and operations.
...
├── docs/                     # Research docs, architecture, security, measurement
├── docker-compose.yml         # Dev environment container orchestrator
├── CONTRIBUTING.md
├── README.md                 # Research-framed overview (this is the default README)
└── README.engineering.md     # This file (engineering walk-through)
```
...
| `OLLAMA_TIMEOUT` | Timeout for Ollama provider calls (added in Workstream 7) | `30` (seconds) |
...
> [!WARNING]
> The dynamic execution tracer runs user-submitted Python code. Although locked down via `sys.settrace()`, instruction caps, and sandboxing, **do not host this application on a public-facing server without additional OS-level container isolation** (such as lightweight Docker sandboxes or microVMs like Firecracker). See [`docs/SECURITY-SANDBOX.md`](./docs/SECURITY-SANDBOX.md) for the residual risk analysis.
